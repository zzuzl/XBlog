		...
		edit : function() {
            ...
            var fire = {
                upload: function () {
                    ...
                    file = file.files[0];
                    var type = fire.getType(file.name);
                    var path = "flashs/articleFlashs/" + Math.random().toString(36).substr(2) + type;
	...
	self.plugin.fileDialog = function(options) {
        ...
        var fire = {
            upload: function () {
                ...
                file = file.files[0];
                var type = fire.getType(file.name);
                var path = "files/articleFiles/" + Math.random().toString(36).substr(2) + type;
                var storageRef = firebase.storage().ref();
                var uploadTask = storageRef.child(path).put(file);
		...
		edit : function() {
            ...
            var fire = {
                upload: function () {
                    ...
                    file = file.files[0];
                    var type = fire.getType(file.name);
                    var path = "medias/articleMedias/" + Math.random().toString(36).substr(2) + type;
                    var storageRef = firebase.storage().ref();
                    var uploadTask = storageRef.child(path).put(file);
